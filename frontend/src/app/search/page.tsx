    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Search by subject, sender, receiver, VEM number, or words inside documents.
        </p>
      </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                  ...
                  >
                    ...
                    {isOcr ? 'Matched Inside Document' : 'Matched in Subject / Sender'}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-slate-400 dark:text-slate-500">
                    {res.ocrRecord && res.ocrRecord.status === 'COMPLETED'
                      ? `Scan accuracy: ${res.ocrRecord.confidence}%`
                      : 'Saved letter'}
                  </span>

                  <div className="flex items-center space-x-2">
                    <a
                      href={getDownloadUrl(res.letter.id)}
                      download
                      className="inline-flex items-center space-x-1 px-2.5 py-1 text-xs font-semibold rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white transition shadow-2xs"
